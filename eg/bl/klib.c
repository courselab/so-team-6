 ...
 *
 * The `fastcall` attribute causes the compiler to pass the first argument (if of integral type)
 * in the register ECX and the second argument (if of integral type) in the register EDX.
 * (https://gcc.gnu.org/onlinedocs/gcc/x86-Function-Attributes.html)
 *
 * The `noinline` attribute specifies to never inline this function during code generation.
 * This is necessary to make sure that the labels `delay_loop` and `delay_end` are unique throughout the program.
 */
